...
this.label_dropped = function (target_obj, label_obj) {
   ...
   if (qwizdata[i_qwiz].qrecord_id && document_qwiz_user_logged_in_b) {
      ...
      var data = {q_and_a_text: q_and_a_text, i_question: i_question, type: 'labeled_diagram', response: label + '\t' + target_label, correct_b: ''};
...
function process_question (i_qwiz, i_question, htm, opening_tags) {
   ...
   for (var i_choice=0; i_choice<n_choices; i_choice++) {
      ...
      if (n_choices > 1) {
         ...
         new_htm += '<span class="qwiz-choice choices-' + qwiz_question + ' choice-' + qwiz_question_choice + '" onclick="' + qname + '.process_choice (\'' + qwiz_question_choice + '\')">\n'
                    + choice_html + '</span>';
      } else {
...
function process_textentry (i_qwiz, i_question, htm, opening_tags) {
   ...
   // If recording, capture text up to first [c] or [c*].
   if (qwizdata[i_qwiz].qrecord_id && document_qwiz_user_logged_in_b) {
      var m = htm.match (/([\s\S]*?)\[c\**\]/m);
      if (m) {
         var q_and_a_text = m[1];
